...
export default function TagBlogs() {
  ...
  const fetchTag = async (slug) => {
    const response = await fetch(
      `https://acetechinside.tech/wp-json/wp/v2/tags?slug=${slug}`
    );
  ...
  // Fetch blogs by tag ID
  const fetchBlogsByTag = async (tagId) => {
    const response = await fetch(
      `https://acetechinside.tech/wp-json/wp/v2/posts?tags=${tagId}&per_page=20&_embed`
    );
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        const tagData = await fetchTag(tagSlug);
        setTag(tagData);

        // Then fetch blogs for this tag
        const blogsData = await fetchBlogsByTag(tagData.id);
        setBlogs(blogsData);
      } catch (err) {
              ...
              >
                <div className="relative overflow-hidden">
                  ...
                  <div className="absolute top-3 right-3">
                    <span className="bg-[#de60ca] text-white px-2 py-1 text-xs font-bebas uppercase tracking-wide rounded">
                      #{tag?.name}
                    </span>
                  </div>
                </div>
